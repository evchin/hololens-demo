	...
	public class CartoonishAvatarSample : GettingStartedSample
	{
		...
		public void ConvertAvatarToObjFormat()
		{
			...
			CoreTools.SaveAvatarMesh(headMeshRenderer, currentAvatarCode, outputObjFile, MeshFileFormat.OBJ, true, false, 
				haircutId: GetCurrentHaircutName(), recolorTexture: false, textureName: GetAdditionalTextureName());

#if UNITY_EDITOR || UNITY_STANDALONE_WIN
			System.Diagnostics.Process.Start(outputObjDir);
#endif
			progressText.text = string.Format("OBJ file was saved in avatar directory");
		}
		...
		public void ExportAvatarAsFbx()
		{
			...
			CoreTools.SaveAvatarMesh(headMeshRenderer, currentAvatarCode, outputFbxFile, MeshFileFormat.FBX, haircutId: GetCurrentHaircutName(), 
				recolorTexture: false, textureName: GetAdditionalTextureName());

#if UNITY_EDITOR || UNITY_STANDALONE_WIN
			System.Diagnostics.Process.Start(exportDir);
#else
			progressText.text = string.Format("FBX file was saved in avatar directory");
#endif
		}
		...
		public void CreateAvatarPrefab()
		{
			...
			AvatarPrefabBuilder.Instance.CreateAvatarPrefab(GameObject.Find(AVATAR_OBJECT_NAME), HEAD_OBJECT_NAME, HAIRCUT_OBJECT_NAME, currentAvatarCode,
				GetCurrentHaircutName(), new List<Type>() { typeof(RotateByMouse) }, textureName: GetAdditionalTextureName());
#endif
		}
