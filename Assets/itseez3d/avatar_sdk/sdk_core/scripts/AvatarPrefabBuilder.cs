...
namespace ItSeez3D.AvatarSdk.Core
{
	public class AvatarPrefabBuilder
	{
		...
		public void CreateAvatarPrefab (GameObject avatarObject, string headObjectName, string haircutObjectName, string avatarId, 
			string haircutId, List<Type> removedObjectsTypes, int levelOfDetails = 0, string textureName = null)
		{
			...
			SaveMeshAndMaterialForAvatarObject(prefabDir, instantiatedAvatarObject, headObjectName, haircutObjectName, avatarId, haircutId, levelOfDetails, textureName);
		...
		protected void SaveMeshAndMaterialForAvatarObject(string prefabDir, GameObject avatarObject, string headObjectName, string haircutObjectName, 
			string avatarId, string haircutId, int levelOfDetails = 0, string textureName = null)
		{
			...
			if (headObject != null)
			{
				...
				headMeshRenderer.sharedMesh = SaveHeadMeshAsFbxAsset(avatarId, meshFilePath, levelOfDetails, textureName);
				headMeshRenderer.sharedMaterial.mainTexture = LoadTextureAsset(textureFilePath);

				string metallicGlossPropertyName = "_MetallicGlossMap";
				if (headMeshRenderer.sharedMaterial.HasProperty(metallicGlossPropertyName))
				{
					Texture2D metallicGlossTexture = headMeshRenderer.sharedMaterial.GetTexture(metallicGlossPropertyName) as Texture2D;
					if (metallicGlossTexture != null)
					{
						string metallicGlossTextureFilename = Path.Combine(prefabDir, "metallic_gloss.png");
						ImageUtils.ExportTexture(metallicGlossTexture, metallicGlossTextureFilename);
						AssetDatabase.Refresh();
						headMeshRenderer.sharedMaterial.SetTexture(metallicGlossPropertyName, LoadTextureAsset(metallicGlossTextureFilename));
					}
				}
		...
		protected Mesh SaveHeadMeshAsFbxAsset(string avatarId, string fbxPath, int levelOfDetails, string textureName)
		{
			CoreTools.SaveAvatarMesh(null, avatarId, fbxPath, MeshFileFormat.FBX, false, true, levelOfDetails: levelOfDetails, textureName: textureName);
			AssetDatabase.Refresh();
