...
namespace ItSeez3D.AvatarSdk.Core
{
	...
	public class FullbodyAvatarLoader
	{
		...
		private BodyAppearanceController bodyAppearanceController = null;
		private Dictionary<string, HaircutAppearanceController> haircuts = new Dictionary<string, HaircutAppearanceController>();
		private Dictionary<string, OutfitAppearanceController> outfits = new Dictionary<string, OutfitAppearanceController>();
		...
		public IEnumerator LoadAvatarAsync(string avatarCode)
		{
			...
			if (ImproveBodyNormals)
				MeshUtils.ImproveNormals(bodyMeshRenderer.sharedMesh);
			...
			List<string> haircutsInDiscreteFiles = fullbodyAvatarProvider.GetHaircutsInDiscreteFiles(avatarCode);
			foreach (string haircutName in haircutsInDiscreteFiles)
				haircuts.Add(haircutName, null);

			List<string> outfitsInDiscreteFiles = fullbodyAvatarProvider.GetOutfitsInDiscreteFiles(avatarCode);
