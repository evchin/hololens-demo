...
namespace ItSeez3D.AvatarSdkSamples.Core
{
	public class AvatarViewer : MonoBehaviour
	{
		...
		protected HaircutRecoloring haircutRecoloring = null;
		...
		{
			var headMeshRenderer = headObject.GetComponent<SkinnedMeshRenderer>();
			headMeshRenderer.material = MaterialAdjuster.GetHeadMaterial(currentAvatarCode, headMeshRenderer.material.mainTexture as Texture2D, shaderType);

			if (haircutObject != null)
			{
				MeshRenderer haircutMeshRenderer = haircutObject.GetComponent<MeshRenderer>();
				haircutMeshRenderer.material = MaterialAdjuster.GetHaircutMaterial(haircutMeshRenderer.material.mainTexture as Texture2D, GetCurrentHaircutName(), shaderType);
